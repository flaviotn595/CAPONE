...
const start = (aruga = new Client()) => {
    ...
    console.log(color(figlet.textSync('CAPONE', { font: 'Ghost', horizontalLayout: 'default' })))
    console.log(color(figlet.textSync('----------------', { horizontalLayout: 'default' })))
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'remove' && event.who !== host) {
			await aruga.sendFileFromUrl(event.chat, profile, 'profile.jpg', '')
            await aruga.sendTextWithMentions(event.chat, `A Deus @${event.who.replace('@c.us', '')}, Sentiremos sua falta✨`)
        }
    })
