...
module.exports = HandleMsg = async (aruga, message) => {
	...
	// Filter Banned People
        ...
        case 'help':
            ...
            break
        case 'menuadmin':
            if (!isGroupMsg) return aruga.reply(from, 'aaf, este comando só pode ser usado dentro do grupo!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Sai dai membro comum so adm pôde usar esse comando!', id)
	...
	case 'brainly':
            ...
            if (args.length >= 2){
			    ...
			    aruga.sendText(from, 'Feito ✅, doe aqui, paypal.me/TheSploit |')
                        } else {
        ...
        case 'nulis':
            ...
            await aruga.sendImage(from, `${nulisp}`, '', 'TOMA AI...', id)
            .catch(() => {
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'Este comando só pode ser usado quando o bot se torna administrador', id)
                    if (isBotGroupAdmins) {
                        aruga
                            ...
                            .then((res) => {
                                aruga.reply(from, `Use com sucesso Revogar Link do Grupo *${prefix}grupolink* para obter o último link de convite de grupo`, id);
                            })
                            .catch((err) => {
                                console.log(`[ERR] ${err}`);
                            });
        ...
        case 'ytmp4':
            ...
            .then(async(res) => {
				...
				await aruga.sendFileFromUrl(from, `${res.result.url}`, '', '', id)
				.catch(() => {
					aruga.reply(from, `URL Ini ${args[0]} Nunca foi baixado antes. O URL será redefinido após 1 hora / 60 minutos`, id)
				})
			})
            break
		case 'fb':
        ...
        case 'anime':
            ...
            if (args[0] == 'random' || args[0] == 'waifu' || args[0] == 'husbu' || args[0] == 'neko') {
                ...
                .then(body => {
                    ...
                    aruga.sendFileFromUrl(from, randomnimex, '', 'TOMA AI..', id)
                })
                .catch(() => {
                    aruga.reply(from, 'Algo está errado', id)
                })
	...
	case 'dewabatch':
        ...
        case 'nekopoi':
            ...
            break
        case 'stalkig':
            if (args.length == 0) return aruga.reply(from, `Para perseguir a conta do Instagram de alguém, digite ${prefix}stalkig [username]\ncontoh: ${prefix}stalkig este preço`, id)
            ...
            break
        case 'wiki':
            if (args.length == 0) return aruga.reply(from, `Para pesquisar uma palavra na wikipedia, digite: ${prefix}wiki [a palavra]`, id)
            ...
            break
        case 'tempo':
            if (args.length == 0) return aruga.reply(from, `Para ver a previsão do tempo para uma área, digite:: ${prefix}tempo [área]`, id)
        ...
        case 'lyrics':
        case 'lirik':
            if (args.length == 0) return aruga.reply(from, `Para pesquisar a letra de uma música, digite: ${prefix}letra [título da música]`, id)
            ...
            break
        case 'chord':
            if (args.length == 0) return aruga.reply(from, `Para pesquisar a letra e os acordes de uma música, digite: ${prefix}chord [título da música]`, id)
        ...
        case 'ss': //jika error silahkan buka file di folder settings/api.json dan ubah apiSS 'API-KEY' yang kalian dapat dari website https://apiflash.com/
            if (args.length == 0) return aruga.reply(from, `Faça um bot para capturar a tela de um \n\n Uso da web:: ${prefix}ss [url]\n\ncontoh: ${prefix}ss http://google.com`, id)
        ...
        case 'whatanime':
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                    ...
                    teks = ''
                    if (similarity < 0.92) {
                    	teks = '*Eu tenho pouca fé nissoi* :\n\n'
                    }
                    teks += `➸ *Title português* : ${title}\n➸ *Title português* : ${title_portugues}\n➸ *Title português* : ${title_portugues
        ...
        // ℂ𝕠𝕞𝕒𝕟𝕕𝕠𝕤 𝕕𝕖 𝕘𝕣𝕦𝕡𝕠 (𝕒𝕡𝕖𝕟𝕒𝕤 𝕒𝕕𝕞 𝕕𝕠 𝕘𝕣𝕦𝕡𝕠)
	    case 'add':
            if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado dentro de grupos!', id)
        ...
        case 'kick':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Falha, este comando só pode ser usado por administradores de grupo!', id)
            if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, adicione o bot como administrador do grupo!', id)
            if (mentionedJidList.length === 0) return aruga.reply(from, 'Desculpe, o formato da mensagem está errado. \Por favor, marque uma ou mais pessoas para serem excluídas', id)
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, 'Desculpe, o formato da mensagem está errado. \Não é possível emitir uma conta de bot sozinho', id)
        ...
        case 'demote':
            ...
            if (mentionedJidList[0] === botNumber) return await aruga.reply(from, 'Desculpe, o formato da mensagem está errado. \ NNão é possível demonstrar a conta do bot', id)
        ...
        case 'everyone':
            ...
            let hehex = '╔══✪〘 OIII SEUS PUTO ADM QUE ATENÇÃO 〙✪══\n'
		...
		case 'simi':
			...
			if (args[0] == 'on') {
                ...
                aruga.reply(from, 'Ative o barco simi-simi!', id)
			} else if (args[0] == 'off') {
        ...
        case 'ban':
            ...
            if (args[0] == 'del') {
             ...
             for (let i = 0; i < mentionedJidList.length; i++) {
                ...
                fs.writeFileSync('./settings/banned.json', JSON.stringify(banned))
                aruga.reply(from, 'Alvo de proibição de sucesso!', id)
                }
            }
            break
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *C A P O N E* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *C A P O N E* 〙\n\n${msg}`)
            }
            aruga.reply(from, 'Sucesso na transmissão', id)
            break
        case 'leaveall': //𝕣𝕖𝕞𝕠𝕧𝕖𝕟𝕕𝕠 𝕓𝕠𝕥𝕤 𝕕𝕖 𝕥𝕠𝕕𝕠𝕤 𝕠𝕤 𝕘𝕣𝕦𝕡𝕠𝕤 𝕖 𝕖𝕩𝕔𝕝𝕦𝕚𝕟𝕕𝕠 𝕠 𝕔𝕙𝕒𝕥
